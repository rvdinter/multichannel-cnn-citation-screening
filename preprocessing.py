...
class Preprocessor:
    ...
    def load_clean_dataset(self, path, sheet, batch_size):
        dataset = self._load_dataset(path, sheet)

        self._create_vocab(dataset)
        self._steps_per_epoch = self.calculate_steps_per_epoch(dataset, batch_size)

        # Clean title and abstract based on vocab, with minimum of 10 occurences per word
        dataset['text'] = dataset['text'].apply(lambda text: self._clean_text(text))
        return dataset, self._steps_per_epoch
    ...
    def _load_dataset(self, path, sheet):
        ...
        df['label'] = df['Included']
        return df[['text', 'label']]

    def split_df_to_balanced_encoded_dataset(self, df, train_indices, test_indices, seed,
                                             buffer_size, batch_size, oversample=True):
        weights = None
        if oversample:
            # Make balanced classes
            weights = [0.5, 0.5]

        # split the dataset into a train, test, and val set
        train = df.iloc[train_indices]
        test = df.iloc[test_indices]
        train, val = train_test_split(train, test_size=0.1, stratify=train['label'], random_state=seed)

        # Fit tokenizers on train data
        # self._fit_tokenizers(train)

        # Separate pos and neg training features and labels
        bool_train_labels = train['label'].values != 0
        pos_features = train[bool_train_labels]['text']
        neg_features = train[~bool_train_labels]['text']

        pos_labels = train[bool_train_labels]['label']
        neg_labels = train[~bool_train_labels]['label']
        ...
        test_ds = self._encoded_dataset(test['text'], test['label'])
        val_ds = self._encoded_dataset(val['text'], val['label'])
    ...
    def _create_embedding_matrix(self, sheet):
        ...
        for i, word in enumerate(self.vocab):
            ...
            try:
                ...
                pass
        os.makedirs('weights/' + sheet, exist_ok=True)
        np.save('weights/' + sheet + '/' + str(self.WV_DIM) + 'd', wv_matrix)
        
    def get_embedding_model(self, path='embeddings/glove.6B.100d.txt'):
